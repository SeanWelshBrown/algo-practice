...
function killProcess(pid, ppid, kill) {
  if (!ppid.includes(kill)) return "No process like that here, bub!";
  
  let results = [kill];
  
  function recursiveFunction(process) {
    if (!ppid.includes(process)) return;
    
    let ppidIndices = [];
    for (let i = 0; i < ppid.length; i++) {
      if (ppid[i] === process) ppidIndices.push(ppid[i]);
    }

    for (let i = 0; i < ppidIndices.length; i++) {
      results.push(pid[i]);
      recursiveFunction(pid[i]); 
    }
  }
  
  recursiveFunction(kill);
  return results;
}